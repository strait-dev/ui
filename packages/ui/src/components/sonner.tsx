...
/**
 * App-level toast host that bridges Sonner with the design-system theme.
 *
 * Render exactly one `<Toaster />` near the root of your layout. Toasts are
 * then triggered imperatively via Sonner's `toast()` helper anywhere in the
 * app without additional setup.
 *
 * @remarks
 ...
 *   `error`, `loading`) is wired to the matching HugeIcons glyph so the
 *   icon language is consistent with the rest of the design system. The
 *   `loading` icon spins via `animate-spin`.
 ...
 * ```tsx
 * // In your root layout:
 * import { Toaster } from "@strait/ui/toaster";
 *
