...
The high-level wrappers (`AreaChart`, `BarChart`, `LineChart`, `PieChart`, `DonutChart`) are exported from `@strait/ui/components/charts`. The lower-level `Chart`, `ChartContainer`, and tooltip/legend primitives are exported from `@strait/ui/components/chart`.
